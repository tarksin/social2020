...
class Message(db.Document):
  ...
  #  oldtext = db.StringField(db_field="t", max_length=1024)
    text = db.StringField(db_field="tx", max_length=None)
    title = db.StringField(db_field="ti", max_length=100)
    ...
    def post_imgsrc(self, image_ts, size):
        if AWS_BUCKET:
            ...
            return url_for('static',
                           filename=os.path.join(STATIC_IMAGE_URL, 'posts', '%s.%s.%s.png' % (self.id, image_ts, size)))
